...
public class FlinkMain extends FlinkMainModel {
    public static void main(String[] args) throws Exception {
        ...
        //3.执行环境设置
        StreamExecutionEnvironment env = StreamEnvBuilder.builder()
        ...
        FlinkKafkaConsumerBase<String> sourceKafka = SourceKafkaBuilder.builder(uat, topicName, groupId, new SimpleStringSchema())
        ...
        SinkFunction<String> esSink = new EsSinkBuilder<>(esHosts, uat.getEsPort(), new ElasticsearchSinkFunction<String>() {
            @Override
        ...
        hosts.add("host1");
        hosts.add("host2");
        SinkFunction<DwdOrderBean> sink = JdbcSink4ClickHouse.sink("insert into tableName (id,name) values (?,?)",
                new JdbcStatementBuilder<DwdOrderBean>() {
                    @Override
                    public void accept(PreparedStatement ps, DwdOrderBean dwdOrderBean) throws SQLException {
                        Field[] fields = dwdOrderBean.getClass().getDeclaredFields();
